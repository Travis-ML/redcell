...
2. **Start the OpenShell gateway:**
   ```bash
   docker compose up -d openshell-gateway     # or just `docker compose up -d` for the whole stack
   ```
   This is a container defined in `docker-compose.yml`, config in
   `openshell/gateway.toml`, published on host port **8080 specifically** — the docker
   compute driver tells sandboxes to call back on the gateway's own port, so remapping
   it leaves them stuck in `Provisioning`. Compose brings up `openshell-jwt-init`
   first (a one-shot container running `openshell/jwt-init.sh` to generate the
   Ed25519 JWT signing keypair the gateway refuses to start without and does not
   create itself) and gates `openshell-gateway` on it via
   `depends_on: condition: service_completed_successfully`, so a plain
   `docker compose up` handles the ordering for you. Running host-mode (`uv run
   redcell serve` outside Compose) still needs the `openshell` CLI on `PATH`
   (`uv tool install -U openshell`) to talk to that same gateway container.
...
its MCP server didn't start (missing runtime or unreachable OpenShell sandbox). See
